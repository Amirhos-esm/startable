...
func main() {
	counter := Counter{}
	counter.SetPreStartCallback(func() error {
	...
	go func() {
		for {
			...
			fmt.Println("isRunning,", counter.IsRunning())
			fmt.Println("isActive,", counter.IsActive())
		}
	}()
	fmt.Println("start,", counter.Start(context.Background()))
